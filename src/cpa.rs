...
impl Cpa {
    ...
    pub fn finalize(&mut self){
        ...
        for i in 0..self.guess_range{
            ...
            map(|x|
            {
                let _sumleakages = self.sum_leakages[x as usize] as f32 / self.len_leakages as f32;
                let _sigleakages = self.sig_leakages[x as usize] as f32 / self.len_leakages as f32;
    ...
    pub fn calculation(&mut self){
        ...
        for i in 0..self.guess_range{
            ...
            self.max_corr[[i as usize, 0]] = *row.iter().max_by(|x, y| x.partial_cmp(y).unwrap()).unwrap();
        }
